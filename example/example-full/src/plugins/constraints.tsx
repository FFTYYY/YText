...
function set_force_sectioner(editor: EditorComponent, slate: SlateReact.ReactEditor): SlateReact.ReactEditor{
    ...
    slate.normalizeNode = (entry: [Slate.Node, number[]]) => {
        ...
        if(path.length == 0){
            ...
            // 开头不是小节线的情况。
            if(nc == 0 || !slate_is_style(node.children[0] ,  "support" , "SectionLine") ){
                editor.add_nodes(editor.get_core().create_support("SectionLine") , [0])
                return 
            }

            // 结尾不是章节线的情况。
            if(!slate_is_style(node.children[nc-1] ,  "support" , "BottomLine")){
                editor.add_nodes(editor.get_core().create_support("BottomLine") , [nc])
                return 
            }
        }

        if(slate_is_style(_node , "support" , "BottomLine")){
